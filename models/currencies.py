...
def currencies_dict(date_now_str: str = '21-06-2023') -> dict[str | None, list[float | str | None]] | str:
    ...
    try:
        ...
        logger.exception(f'CurrencyFileError: {str(e)}')
        return 'Sorry, but we have not information about actual rate currency.'
    except ET.ParseError as e:
        logger.exception(f'CurrencyParseError: {str(e)}')
        return 'Sorry, but we have not information about actual rate currency.'

    root = tree.getroot()
    currencies = {}
    for child in root:
        currencies[child[1].text] = [float(child[i].text.replace(',', '.')) if child[i].tag == 'Value'else child[i].text for i in range(len(child)) ]
    return currencies


if __name__ == '__main__':
    date_now_str = datetime.datetime.now().strftime('%d-%m-%Y')
    print(currencies_dict(date_now_str))
